...
作为中期检查的重要组成部分，本项目不仅关注算法研究，还致力于**离散数学智慧课程**的建设：

- **核心目标**：构建结构化的知识体系，将离散数学（博弈论、图论、集合论等）的基础理论与空间演化博弈的科研实践相结合。
- **知识图谱**：详细梳理了离散数学课程的知识点，并建立了概念间的关联（见`中期检查/离散数学一/二的知识图谱.jpg`），为后续的自动化问答（Q&A）和个性化学习路径推荐提供基础。
- **资源整合**：配套建设了专业的问答库（`问答库建设.xlsx`），旨在打造科研与教学深度融合的“智慧课程”原型。

## 📂 项目结构

- **`中期检查/` (Midterm Inspection)**:
  - **代码复现**: `1.py`, `2.py`, `3.py`。这部分代码主要用于复现经典文献中的基础实验结果，为本项目提供对比基线。
  - **知识图谱建设**: 包含《离散数学》课程的知识点梳理（`.xlsx`）、知识图谱可视化（`.jpg`）以及相关的建设方案（`知识图谱.pptx`）。
  - **问答库与规划**: 包含`问答库建设.xlsx`及智慧课程、总体规划相关的演示文档（`.pptx`）。
  - **算法框架**: 包含多个版本的算法整体框架图（`.png`），展示了研究思路的演进。
- **根目录文件**:
